...
//glm::vec3 lightDiffuse(1.5f, 1.5f, 1.5f);
//glm::vec3 lightSpecular(0.45f, 0.45f, 0.45f);
...
Material generalMaterial(glm::vec3(1.0f),0.5f,0.8f,1.0f);
// esc
bool esc_is_pressed = false;

bool use_ambientCube = true;

int main() 
{
    GLFWwindow* window = demoInit();
    ...
    glm::vec3 wall_material_objectColor[] = {
            ...
            //glm::vec3(1.0f,1.0f,0.0f)/PI,glm::vec3(1.0f,1.0f,1.0f)/PI,glm::vec3(0.0f,1.0f,1.0f)/PI,

            glm::vec3(0.5f,0.0f,0.0f),glm::vec3(0.5f,0.5f,0.5f),glm::vec3(0.0f,0.5f,0.0f),
            glm::vec3(0.5f,0.5f,0.5f),glm::vec3(0.0f,0.0f,0.5f),glm::vec3(0.5f,0.5f,0.5f)
    };
    ...
    std::vector<TrianglePlane> trianglePlanes =
        GetTrianglePlanes(wall_num,wallVertices,wall_material_objectColor, wallPositions,wallAngle, wallAxis, 
            cube_num, cubeVertices, cube_material_objectColor, cubePositions,generalMaterial);
    ...
    lightingShader.setVec3("material.albedo", generalMaterial.albedo);
    lightingShader.setFloat("material.metallic", generalMaterial.metallic);
    lightingShader.setFloat("material.roughness", generalMaterial.roughness);
    lightingShader.setFloat("material.ao", generalMaterial.ao);
    //-------------------------------------------
    for (int i = 0; i < 4; i++)
    {
    ...
    CubeShader.setVec3("material.albedo", generalMaterial.albedo);
    CubeShader.setFloat("material.metallic", generalMaterial.metallic);
    CubeShader.setFloat("material.roughness", generalMaterial.roughness);
    CubeShader.setFloat("material.ao", generalMaterial.ao);
    //-------------------------------------------
    for (int i = 0; i < 4; i++)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < wall_num; i++)
        {
            ...
            lightingShader.setVec3("material.albedo", wall_material_objectColor[i]);
        ...
        if (bands != lastbands)
        {
            ...
            for (auto& probe : probes)
            {
                ...
                glm::vec3* coeffs = ProjectIrradianceFunctionSH(probe, bands);
                //glm::vec3* coeffs = ProjectRadianceFunctionSH(probe, bands);
                all_coeffs.push_back(coeffs);
            }
            ...
            rightKeyPressed|| leftKeyPressed||forwardKeyPressed||backKeyPressed||pageUpKeyPressed || pageDownKeyPressed)
        {
            ...
            for (int count=0; (count<step_length)&&(probe_compute_offset<probes.size()); count++, probe_compute_offset++)
            {
                ...
                //glm::vec3* coeffs = ProjectRadianceFunctionSH(probes[probe_compute_offset], bands);
                //all_coeffs.push_back(coeffs);
...
GLFWwindow* demoInit()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "PBR_PRTdemo", NULL, NULL);
    if (window == NULL)
    {
...
void actionKeyPressed(glm::vec3& lightPos, Light& light)
{
    if (dynamic)
    {
        lightPos = glm::vec3(0.0f, 5.0f, 0.0f) + glm::vec3(sin(glfwGetTime()*0.1) * 6.0f, 0.0f, 0.0f);
    ...
    {
        if (light.color.x < 500.0f)
        {
            light.color += glm::vec3(10.f);
        }
    }
