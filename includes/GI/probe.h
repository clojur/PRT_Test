...
class Material {
public:
	unsigned int sampler2Ddiffuse;
	unsigned int sampler2Dspecular;
	glm::vec3 objectColor;
	float shininess;
	Material() {}
	Material(glm::vec3 color, float shini = 16.0f, unsigned int diff = 0, unsigned int specu = 1)
		:objectColor(color), shininess(shini), sampler2Ddiffuse(diff), sampler2Dspecular(specu) {}
};
...
{
	// ambient
	glm::vec3 ambient = light.ambient * material.objectColor;
	...
	if (glm::dot(norm, viewDir) <= 0) return ambient;

	// diffuse 
	float diff = std::max(glm::dot(norm, lightDir), 0.0f);
	glm::vec3 diffuse = light.diffuse * diff * material.objectColor;

	// specular
	glm::vec3 reflectDir = reflect(-lightDir, norm);
	float spec = std::pow(std::max(glm::dot(viewDir, reflectDir), 0.0f), material.shininess);
	if (glm::dot(norm, lightDir) <= 0) spec = 0.0f;
	glm::vec3 specular = light.specular * spec * material.objectColor;
	...
	if (probe_useShadow)
	{
		...
		for (TrianglePlane& trianglePlane : trianglePlanes)
		{
			...
			{
				//glm::vec3 FragColor = (ambient + diffuse + specular)*0.5f;
				glm::vec3 FragColor = ambient;
				return FragColor;
			}
		}
	}
	
	glm::vec3 FragColor = ambient+ diffuse + specular;
...
public:
	...
	////生成均匀分布的方向
	//std::vector<glm::vec3> genAverDir(int number=10) {
	//	//srand(time(NULL));
	//	float rangeTheta = PI;
	...
	//	return dirs;
	//}
	//void GenerateRays(int number = 10)
	//{
	//	//srand(time(NULL));
	//	float rangeTheta = PI;
	//	//球面坐标系
		...
		for (int i = 0; i < rays.size(); i++)
		{
			...
			for (int j = 0; j < rays.size(); j++)
			{
				...
				if (costmp > 0.0f)
				{
					...
					irradiance += costmp * surfels[j].color;//*sintmp
					count++;
				}
			}
			irradiance = irradiance * (4 * PI / (float)count);
...
std::vector<TrianglePlane> GetTrianglePlanes(int wall_num, float wallVertices[], glm::vec3 wall_material_objectColor[],
	...
	int cube_num, float cubeVertices[], glm::vec3 cube_material_objectColor[], glm::vec3 cubePositions[])
{
	...
	for (int i = 0; i < wall_num; i++)
	{
		...
		for (int j = 0; j < 12; j++)
		{
			...
			wallTrianglePlanes[i * 12 + j].material = Material(wall_material_objectColor[i]);
		}
	}
	...
	for (int i = 0; i < cube_num; i++)
	{
		...
		for (int j = 0; j < 12; j++)
		{
			...
			cubeTrianglePlanes[i * 12 + j].material = Material(cube_material_objectColor[i]);
		}
	}
